...
export const GlobalDataProvider = ({ children }) => {
  ...
  const fetchProcurement = React.useCallback(async () => {
    try {
      ...
      if (pos.data?.success)
        setPurchaseOrders(
          pos.data.data.map((po) => ({ ...po, items: parsePOItems(po.items) })),
        );
    } catch (e) {
  ...
  const fetchTickets = React.useCallback(async () => {
    try {
      ...
      if (eventsData.data?.success) {
        ...
        setEvents(filterDataForCurrentUser(mappedEvents));
      }
      if (guestReqs.data?.success) {
        ...
        setGuestRequests(filterDataForCurrentUser(mappedGuest));
      }
    } catch (e) {
  ...
  const updatePurchaseRequest = async (updated) => {
    try {
      ...
      addLog({
        action: "Request Updated",
        detail: `Modified procurement request ${updated.id}.`,
        type: "system",
      });
    } catch (error) {
