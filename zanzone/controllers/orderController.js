    ...
    try {
        ...
        if (isCustomerRole) {
            ...
            if (!resolvedCustomerId) {
                // JWT payload doesn't contain name/phone, so fetch from DB fallback
                const [userRows] = await db.query(
                    `SELECT name, email, phone FROM users WHERE id = ? LIMIT 1`,
                    [req.user.id]
                );
                const dbUser = userRows[0] || {};
                const finalName = dbUser.name || req.user.email || 'Customer';
                const finalPhone = dbUser.phone || null;
                ...
                const [newCust] = await db.query(
                    ...
                    [companyId, finalName, dbUser.email || req.user.email || null, finalPhone, finalName, null, req.user.id]
                );
        ...
        for (const item of parsedItems) {
            ...
            const safeName = item.name || item.product_name || 'Unknown Item';
            await db.query(
                `INSERT INTO order_items (order_id, product_name, category, qty, unit_price, total_price) VALUES (?, ?, ?, ?, ?, ?)`,
                [orderId, safeName, item.category || null, item.qty || item.quantity || 1, item.price || item.unit_price || 0, itemTotal]
            );
        }
